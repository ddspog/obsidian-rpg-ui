---
cssclasses:
  - note-feature
source: From **D&D 5e** "Beyond Damage Dice 1" book by **Kobold Press**
---
# Concealed Sidearm
```rpg rule.content
name: Concealed Sidearm
---
Instead of making a Dexterity ([[Stealth]]) check to hide yourself, you may make a Dexterity ([[Sleight of Hand]]) check—contested by an active or passive Wisdom ([[Perception]]) check—to conceal your [[Crossbow, hand|hand crossbow]]. As an action, you may make a weapon attack with a concealed crossbow against a creature that has not yet acted in combat. This attack has advantage.
```
```rpg rule.related
view: footer
entries:
  - `@[[Weapons]].bare(options)`
```