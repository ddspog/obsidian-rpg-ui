...
import rolls from "../blocks/character/rolls";
...
const character = CreateEntity<CharacterEntity>(async ({ wiki }) => {
  ...
  const refDocs = [...refLoads.flatMap((r) => r.docs ?? []), ...parentLoads.flatMap((r) => r.docs ?? [])];
  ...
  const pushIndexDoc = (d: any) => {
    ...
    const frontmatterTags: string[] = Array.isArray(d?.$tags) ? d.$tags.map((t: string) => t.replace(/^#/, "")) : [];
  ...
  for (const d of [...(skillDocs ?? []), ...(languageDocs ?? []), ...(talentDocs ?? []), ...refDocs]) {
    pushIndexDoc(d);
  }
  ...
  const toDefaultFeature =
    (type: "action" | "reaction" | "bonus") =>
    (d: any): FeatureEntry => ({
      $name: d?.$name ?? "",
      $contents: d?.$contents ?? "",
      type,
  ...
  const collectEquippedPersonalTraits = (
    ...
    const absorb = (source: string, traits: Record<string, string[]>): void => {
  ...
  return {
    ...
    expressions: {
      CharacterLevel: (_, { blocks }) => {
        ...
        const header = (blocks.header ?? (blocks as unknown as { sheet?: HeaderProps }).sheet) as
          | HeaderProps
          | undefined;
      ...
      ModifierTotal: ([{ attribute, proficiency, bonus }], { blocks, lookup, expressions }) => {
        ...
        const statsSource = (blocks.stats ?? (blocks as unknown as { sheet?: StatsProps }).sheet) as
        ...
        const header = (blocks.header ?? (blocks as unknown as { sheet?: HeaderProps }).sheet) as
          | HeaderProps
          | undefined;
        const featuresBlock = (blocks.features ?? (blocks as unknown as { sheet?: FeaturesBlockData }).sheet) as
          | FeaturesBlockData
          | undefined;
        const asiTraits: string[] =
          lookup.$features?.(header, featuresBlock?.choices, featuresBlock?.additional)?.traits?.["Ability Scores"] ??
          [];
