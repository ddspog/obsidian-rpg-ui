  ...
  return (
    <div className="initiative-list">
      <div className="initiative-items">
        {props.sortedItems.map(({ item, index, initiative }) => (
          <InitiativeRow
            key={index}
            item={item}
            index={index}
            initiative={initiative}
            isActive={index === props.state.activeIndex}
            state={props.state}
            inputValue={props.inputValue}
            onInitiativeChange={(value) => props.onInitiativeChange(index, value)}
            onInputChange={props.onInputChange}
            onDamage={(monsterKey) => props.onDamage(index, monsterKey)}
            onHeal={(monsterKey) => props.onHeal(index, monsterKey)}
          />
        ))}
      </div>
    </div>
