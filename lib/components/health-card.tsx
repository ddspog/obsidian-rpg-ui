...
export function HealthCard(props: HealthCardProps) {
  ...
  const healthPercentage = calculateHealthPercentage(props.state.current, maxHealth);

  // Use custom hook for all state management and event handlers
  const { inputValue, setInputValue, handleHeal, handleDamage, handleTempHP, toggleHitDie, toggleDeathSave } =
