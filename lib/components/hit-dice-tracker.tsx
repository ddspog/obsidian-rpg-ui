...
import { isSingleHitDiceState, isMultiHitDiceState } from "lib/domains/healthpoints";
    ...
    return (
      <div className="hit-dice-list">
        {props.hitDice.map((hd) => {
          ...
          if (props.hasSingleHitDice && isSingleHitDiceState(props.state)) {
          ...
          for (let i = 0; i < hd.value; i++) {
            hitDiceArray.push(
              <Checkbox
                ...
                onChange={() => props.onToggle(props.hasSingleHitDice ? null : hd.dice, i)}
              />
            );
          }
