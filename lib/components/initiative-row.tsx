...
import { getMaxHp, itemHashKey } from "lib/domains/initiative";
import { MonsterHpDisplay } from "lib/components/monster-hp-display";
import { getHealthStatusClass } from "lib/domains/initiative-utils";
...
export function InitiativeRow(props: InitiativeRowProps) {
  ...
  const isGroup = hasHp && typeof props.item.hp === "object" && Object.keys(props.item.hp).length > 1;
      ...
      <div className={`initiative-hp-inline ${statusClass}`}>
        ...
        <div className="initiative-hp-controls">
          ...
          <button
            ...
            onClick={() => props.onDamage(monsterKey)}
            title="Damage"
          >
            −
          </button>
          <button
            ...
            onClick={() => props.onHeal(monsterKey)}
            title="Heal"
          >
            +
          </button>
  ...
  const renderGroupHp = () => {
    return (
      <div className="initiative-group-hp">
        {Object.entries(props.item.hp as Record<string, number>).map(([key]) => {
          const currentHp = itemHp[key] || 0;
          ...
          return (
            <MonsterHpDisplay
              ...
              onDamage={() => props.onDamage(key)}
              onHeal={() => props.onHeal(key)}
            />
          );
        })}
      </div>
  ...
  return (
    <div
      className={`initiative-item ${props.isActive ? "initiative-item-active" : ""} ${
    ...
    >
      <div className="initiative-item-main">
        <div className="initiative-roll">
          <input
            ...
            onChange={(e) => props.onInitiativeChange(e.target.value)}
        ...
        <div>
          <div className="initiative-name">
            {props.item.link ? (
              <a href={props.item.link} className="initiative-link">
                {props.item.name}
              </a>
            ) : (
              props.item.name
          ...
          <div className="initiative-ac">
            AC: <span className="initiative-ac-value">{props.item.ac}</span>
          </div>
        </div>
      ...
      {isGroup && (
        <>
          ...
          <div className="initiative-group-container">{renderGroupHp()}</div>
        </>
      )}
    </div>
  );
