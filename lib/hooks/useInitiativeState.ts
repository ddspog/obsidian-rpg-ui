...
import { calculateNextTurn, calculatePreviousTurn } from "lib/domains/initiative-utils";
...
export function useInitiativeState(
  ...
  const sortedItems = getSortedInitiativeItems(static_.items, state.initiatives);
  ...
  const handleNext = () => {
    ...
    const { nextActiveIndex, newRound, roundIncremented } = calculateNextTurn(
      sortedItems,
      state.activeIndex,
      state.round
    );
  ...
  const handlePrev = () => {
    ...
    const { prevActiveIndex, newRound } = calculatePreviousTurn(sortedItems, state.activeIndex, state.round);
