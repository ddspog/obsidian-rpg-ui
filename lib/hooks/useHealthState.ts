...
import { isMultiHitDiceState, isSingleHitDiceState } from "lib/domains/healthpoints";
import { calculateHealing, calculateDamage, calculateTempHP } from "lib/domains/health-utils";
import { calculateNewUsedCount, updateDeathSaveSuccesses, updateDeathSaveFailures } from "lib/domains/death-save-utils";

export function useHealthState(state: HealthState, maxHealth: number, onStateChange: (newState: HealthState) => void) {
  const [inputValue, setInputValue] = useState("1");

  // Handle healing action
  const handleHeal = () => {
    const value = parseInt(inputValue) || 0;
    const { newCurrent, resetDeathSaves } = calculateHealing(state.current, value, maxHealth);
  ...
  const handleDamage = () => {
    const value = parseInt(inputValue) || 0;
    const { newCurrent, newTemporary } = calculateDamage(state.current, state.temporary, value);
  ...
  const toggleHitDie = (diceType: string | null, index: number) => {
    if (!diceType && isSingleHitDiceState(state)) {
      ...
      const newHitDiceUsed = calculateNewUsedCount(state.hitdiceUsed, index);
  ...
  const toggleDeathSave = (type: "success" | "failure", index: number) => {
    ...
    if (type === "success") {
      newState.deathSaveSuccesses = updateDeathSaveSuccesses(state.deathSaveSuccesses, index);
    } else {
      newState.deathSaveFailures = updateDeathSaveFailures(state.deathSaveFailures, index);
    }
