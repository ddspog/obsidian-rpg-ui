...
import type { LonelogEntry, PersistentTag, EntityDelta, StateChange, ProgressChange, ThreadChange } from "./types";
import {
...
export function extractDeltas(entries: LonelogEntry[]): {
  ...
  for (const entry of entries) {
    if (entry.type === "consequence") {
      processConsequenceTags(entry.tags, registry, entityDeltas, progressChanges, threadChanges);
    }
  }
 ...
 */
export function accumulateDeltas(allDeltas: EntityDelta[]): Map<string, EntityDelta> {
