 ...
 */
function parseHPChange(match: RegExpMatchArray): { type: "hp"; delta: number } {
 ...
 */
function parseStatChange(match: RegExpMatchArray): { type: "stat"; stat: string; delta: number } {
  const stat = match[1];
 ...
 */
function parseStatusTransition(match: RegExpMatchArray): { type: "status"; from: string; to: string } {
...
export function getFinalStatus(changes: StateChange[]): string | null {
  const statusChanges = changes.filter(
    (c): c is { type: "status"; from: string | null; to: string } => c.type === "status"
  );
